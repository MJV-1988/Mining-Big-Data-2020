...
def k_means(k, start_indexes, data):
    ...
    centroid_df = data.groupby('Cluster').mean()
    ...
    while not converged:
        ...
        for i in range(0, m):
            ...
            for j in range(1, k):
                ...
                if distance < min_distance:
                    min_distance = distance
                    data.at[i,"Cluster_New"] = j



        # Recalculate the cluster averages
        centroid_df = data.groupby('Cluster_New').mean()
        centroid_df = centroid_df.iloc[:, :-1]

        # Check to see if the new clusters are different from the old
        if( data["Cluster_New"].equals(iris_df["Cluster"]) ):
            converged = True
        else:
    ...
    for i in range(0, m):
        ...
        # Get the cluster
        cluster_i = data.at[i,"Cluster"].astype(int)
...
def main ():
    ...
    # Drop the label column and add columns for cluster assignment
    # at start and end of each iteration
    ...
    # Create dataframe for holding the average distance to centroid
    avg_distance_df = pd.DataFrame(columns=['k value','Average Distance'])

    for i in range(2, 10):
        avg_distance = k_means(i, start_indexes, iris_df)
